...
class Kakyo {
    ...
    /**
     * Get the currently active default locale for this Kakyo instance
     * @returns {string} The currently active default locale
     */
    getDefaultLocale() {
    ...
    /**
     * Set a new default locale for the current Kakyo instance
     * @param {string} locale The new default locale. WARNING: Locales are cAsE sEnSiTiVe!
     */
    setDefaultLocale(locale) {
        if (!this._localizations.hasOwnProperty(locale)) {
            throw new Error(`Cannot set default locale to "${locale}" because it doesn't exist within the localisation files! Typo? Locales are cAsE sEnSiTiVe! Or run "reload()" on your Kakyo instance if new localisations were added beforehand.`);
        }
