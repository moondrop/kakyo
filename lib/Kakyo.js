...
class Kakyo {
    ...
    constructor(folderPath, options = {}) {
        ...
        this.defaultLocale = options.defaultLocale || null;
        this.localizations = {};
        this._initialize();
    }
     ...
     */
    _initialize() {
        const localizationFiles = Util.readDirectory(this.folderPath);

        for (let locale of localizationFiles) {
            const localeObject = require(locale);

            for (let localeProp in localeObject) {
                if (!this.localizations.hasOwnProperty(localeProp)) {
                    Object.assign(this.localizations, { [localeProp]: localeObject[localeProp] });
                }
            }
        }
    }
    ...
    get(responseKey, data, locale) {
        if (!locale && this.defaultLocale === null) {
            throw new Error(`No locale was passed and no default locale was set.`)
        }

        const computedLocale = (locale) ? locale : this.defaultLocale;
        const responseLocale = this.localizations[computedLocale];
     ...
     * @async
     * @returns {Promise<void>} Resolves with an empty promise upon successful reload
     */
    async reload() {
        return new Promise((resolve, reject) => {
            try {
                ...
                Object.keys(require.cache).forEach((key) => {
                    if (require.cache[key].path === this.folderPath) {
